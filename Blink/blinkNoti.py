...
class BlinkNotifer:
    def __init__(self, svm_model_path, detector):
    ...
    def frameProcess(self, frame, cvt_code):
    ...
    def landmarksDetection(self, meshmap, img, results, draw=False):  # meshmap is an array to store the coords, img is the frame, results is the results from mediapipe facemesh detector
    ...
    def detect_blinks(self):
       ...
       #create display window
        ...
        while video_stream.isOpened():
            ...
            self.frameProcess(frame, cv.COLOR_BGR2RGB)  # double the frame height and width as well as convert frame to RGB. WHY RESIZE?
            frame = cv.flip(frame, 1)

            #get the facemesh results
            ...
            if results.face_landmarks:
                self.landmarksDetection(self.mesh_coord, frame, results, False)  # fill in the mesh_coord array with pixel coords of eyes landmarks on the facemesh from the result
...
blink_notifier = BlinkNotifer(svm_model_path="ear_svm_maf_ec_model.pkl", detector = detector)
